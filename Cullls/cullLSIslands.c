    ...
    void cullLSIslands(CullLSParams *cullPar)
{
	...
	int nx,ny;              /* Size in range and azimuth */
	...
	*/
	...
	for(i=0; i < ny; i++) 
		for( j=0; j < nx; j++ ) {
			mask[i][j]= VALIDPIXEL;
			if(cullPar->XS[i][j] >= (float)(-LARGEINT+1000) ) mask[i][j] +=REGION;
		}
	/*
	  Label image
	*/
	labels  = (int **) malloc(ny * sizeof(int *) ); 
	buf=   malloc(ny*nx*sizeof(int));
	for(i=0; i < ny; i++) labels[i] = (int *) &(buf[i*nx]);
	/*
	...
	*/
	...
	/*
	  ...
	  for(i=0; i < ny; i++) fwriteBS(uwImage.labels[i],sizeof(long int),nx,fp,INT32FLAG); fclose(fp);
	*/
	/*
