...
void cullLSData(CullLSParams *cullPar)
{
	...
	for(i=0; i < ny; i++) {
		for(j=0; j < nx ; j++) {
			...
			nTotal++;
			if( (cullPar->matches.type[i][j] & NODATATOUSE) == 0) nWithData++;
		}
	}
	/* 
	...
	fprintf(stderr,"n initial %f %f %i %i %i %i\n",(double)nGoodPts/(double)(nCulled+nGoodPts),(float)nGoodPts/(float)nWithData, nTotal,nWithData,ngood,nCulled);
		...
		for(j=0; j < nx; j++) {
				...
				for(jj=j1; jj <= j2; jj++ ) {
					if(cullPar->matches.X[ii][jj] > (1-LARGEINT) &&
					   cullPar->matches.Y[ii][jj] > (1-LARGEINT) ) {
						...
						if( ! (cullPar->matches.type[ii][jj] & SLOWVEL)) slowFlag=FALSE;
					} /* Endif cullPar.. */
				} /* End jj */
			} /* End ii */
			if(ngood > cullPar->nGood && cullPar->matches.Y[i][j] > (NODATA+1)  && cullPar->matches.Y[i][j] > (NODATA+1)   ) {
				...
				if(slowFlag == TRUE) {
					...
					maxX=40.0/365.  * cullPar->fitDat.deltaT; maxY=maxX;
