...
void corrTrackFast(TrackParams *trackPar)
{
	...
	*/
	...
	for (i = 0; i < trackPar->wRa * OS; i++)
		for (j = 0; j < trackPar->wAa * OS; j++)
			dataS[j][i] = 0;
	...
	*/
	...
	for (i = 0; i < trackPar->nA; i++)
	{
		...
		fprintf(stderr,
				...
				"nAmp %8i %4.1f (%7i-%7i) nFail %7i %4.1f cAvg(line) %4.2f %i -- %5i --",
				a1,
				nTot, trackPar->nComplex + trackPar->nAmp + trackPar->nAmpL, 100. * (double)(trackPar->nComplex + trackPar->nAmp + trackPar->nAmpL) / (double)nTot,
	...
	*/
	...
	if (a2 < imageBuf2.firstRow || (a2 + wAa) > imageBuf2.lastRow)
	{
		...
		if (trackPar->floatFlag == TRUE)
			...
			freadBS((void *)imageBuf2.buf[0], sSize, s2, fp2, INT16FLAG);
	...
	*/
	for (i = 0; i < wAa; i++)
	{
		...
		if (i1 > NBUFFERLINES || i2 > NBUFFERLINES)
			error("Buffer exceeded in getCorrPatchesFast \n");
		for (j = 0; j < wRa; j++)
		{
			j1 = r1 + j;
			j2 = r2 + j;
	...
	for (i = 0; i < wA2; i++)
	{
		...
		for (j = 0; j < wR2; j++)
		{
			...
			if (trackPar->navgA > 1 || trackPar->navgR > 1)
			{
				...
				dataR[i][j] = (im2[i1][j1].re * im2[i1][j1].re + im2[i1][j1].im * im2[i1][j1].im) * scale;
		}
	}
...
/*************************************************************************
   Find inital shift
   ************************************************************************* */
static void findImage2Pos(int32_t r1, int32_t a1, TrackParams *trackPar,
...
{
	extern float **dataS, **dataR;
	int32_t countR, countS; /* Count of good pixel and reference data */
	int32_t ls, ss;
	int32_t lr, sr;
	int32_t i, j;
	int32_t wR2, wA2;
...
/***********************************************************
   Main code to execute point32_t correlation
************************************************************/
