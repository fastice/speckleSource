...
void parseTrack(char *parFile, TrackParams *trackPar)
{
	...
	*/
	...
	if((trackPar->initialOffsetFile=rdfValue(rdfParams,"initialoffsetfile"))
	   ==NULL) error("parseTrack: Missing initialoffsetfile");
    
	trackPar->intGeodat=NULL;
	trackPar->baseParams=NULL;
	if( (trackPar->intFile=rdfValue(rdfParams,"intfile")) == NULL) trackPar->intFlag=FALSE; 
	else {
		trackPar->intFlag=TRUE;
		if( (trackPar->intGeodat=rdfValue(rdfParams,"intgeodat")) == NULL) 
			error("parseTrack: Missing intgeodat");
	}
	if( (trackPar->baseParams=rdfValue(rdfParams,"baseparams")) ==NULL)  	trackPar->baseFlag=FALSE;
	else {
	...
	if(trackPar->noComplex==TRUE) trackPar->baseFlag=FALSE;
	...
	} else error("parseTrack: missing ouput file");
			...
			void *d1,void *d2,void *d3, int n)
{
	...
	if(value == NULL) error("%s not specified", keyword);
